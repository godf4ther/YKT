    ...
    for (int i = 0; i < month; i++) {
        ...
        for (int j = 0; j < (daysInThisMounth > 29 && (firstDayInThisMounth == 6 || firstDayInThisMounth == 5) ? 42 : 35); j++) {
            ...
            if (j < firstDayInThisMounth || j > daysInThisMounth + firstDayInThisMounth - 1) {
                ...
                NSString *dateStr = [NSString stringWithFormat:@"%zd-%02zd-%02ld",model.year, model.month, model.day.integerValue];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!model.activityColor) {
        ...
        if ([self.delegate respondsToSelector:@selector(calenderView:dateString:)]) {
            [self.delegate calenderView:indexPath dateString:[NSString stringWithFormat:@"%zd-%02ld-%02ld", model.year, model.month, [model.day integerValue]]];
        }
...
- (CalenderWeekView *)weekView {
    if (_weekView == nil) {
        ...
        _weekView.dataSource = self.weekArray ? self.weekArray : @[@"周日", @"周一", @"周二", @"周三", @"周四", @"周五", @"周六"];
    }
    return _weekView;
}
